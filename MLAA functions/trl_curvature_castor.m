 function ni = trl_curvature_castor(yi, bi, ri, li, ctype)
...
%% convert to single
yi=single(yi);  bi=single(bi);  ri=single(ri);
li=single(li);
%%
...
%
% trl_h_dh()
% transmission Poisson likelihood functionmax
%
...
function out=get_index(y,ind)
    threshold=5e7; % define maximum length of index as 5e7
    M=length(ind);
    N=floor(M/threshold); % define maximum length of index as 5e7
    out=zeros(M,1);
    if(N>0)
        for i=1:N
            id_start=threshold*(i-1)+1;
            id_end=threshold*i;
            out(id_start:id_end)=y(ind(id_start:id_end));   
        end
        out(threshold*N+1:end)=y(ind(threshold*N+1:end));  
    else
        out(1:end)=y(ind(1:end)); 
    end    
end

function out=assign_val(y,ind,in)
    threshold=5e7; % define maximum length of index as 5e7
    M=length(ind);
    N=floor(M/threshold); % define maximum length of index as 5e7
    out=in;
    ...
    for i=1:N
        id_start=threshold*(i-1)+1;
        id_end=threshold*i;
        out(ind(id_start:id_end))=y(id_start:id_end);
    end
        out(ind(threshold*N+1:end))=y(threshold*N+1:end); 
    else
        out(ind)=y;
    end   
