function [xt, li, L] = emis_ml_em_castor(yi, ci, G, Gopt, xt, ri, maxit, li, gg)
...
if nargin<8 | isempty(li)
    li = proj_forw_CASTOR(G, Gopt, xt, numfrm);
end
if nargin<9 | isempty(gg)
    gg = proj_back_CASTOR(G, Gopt, ci, numfrm);
end
if size(gg,2)<numfrm
    gg = repmat(gg,[1 numfrm]);
end
%% convert to single
yi=single(yi);  ci=single(ci);  G=single(G);    xt=single(xt);
ri=single(ri);  li=single(li);  gg=single(gg);
%%
% mask
mask = gg>0;
xt(~mask) = 0;
...
for it = 1:maxit
    ...
    yb = ci.*li + ri;
    yr = yi./yb;
    xb = proj_back_CASTOR(G, Gopt, ci.*yr, numfrm);
    xt = xt./gg .* xb;
    xt(gg==0) = 0;
